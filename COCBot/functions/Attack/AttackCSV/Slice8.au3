		...
		If $pixel[0] <= $TopX Then
			If $pixel[1] <= $LeftY Then
				...
				If $pixel[0] <= $TLX Then ; slice 6 external
					If ($TLX - $pixel[0]) / ($TLX - $Left) + ($LeftY - $pixel[1]) / ($LeftY - $TLY) <= 1 Then
						Return "6E"
					Else
						Return "0_6E"
					EndIf
				Else
					If ($pixel[0] - $TLX) / ($TopX - $TLX) + ($LeftY - $pixel[1]) / ($LeftY - $TLY) <= 1 Then
						Return "6_I"
					Else
						If ($TopX - $pixel[0]) / ($TopX - $TLX) + Abs($TLY - $pixel[1]) / ($LeftY - $TLY) <= 1 Then
							If $pixel[1] <= $TLY Then
								Return "5_E"
							Else
								Return "5_I"
			...
			Else
				...
				If $pixel[0] <= $BLX Then ; slice 7 external
					If ($BLX - $pixel[0]) / ($BLX - $Left) + ($pixel[1] - $LeftY) / ($BLY - $LeftY) <= 1 Then
						Return "7_E"
					Else
						Return "0_7"
					EndIf
				Else
					If ($pixel[0] - $BLX) / ($TopX - $BLX) + ($pixel[1] - $LeftY) / ($BLY - $LeftY) <= 1 Then
						Return "7_I"
					Else
						If ($TopX - $pixel[0]) / ($TopX - $BLX) + Abs($BLY - $pixel[1]) / ($BLY - $LeftY) <= 1 Then
							If $pixel[1] <= $BLY Then
								Return "8_I"
							Else
								Return "8_E"
		...
		Else
			If $pixel[1] <= $RightY Then
				...
				If $pixel[0] > $TRX Then ; slice 3 external
					If ($pixel[0] - $TRX) / ($Right - $TRX) + ($RightY - $pixel[1]) / ($RightY - $TRY) <= 1 Then
						Return "3_E"
					Else
						Return "0_3"
					EndIf
				Else
					If ($TRX - $pixel[0]) / ($TRX - $TopX) + ($RightY - $pixel[1]) / ($RightY - $TRY) <= 1 Then
						Return "3_I"
					Else
						If ($pixel[0] - $TopX) / ($TRX - $TopX) + Abs($TRY - $pixel[1]) / ($RightY - $TRY) <= 1 Then
							If $pixel[1] <= $TRY Then
								Return "4_E"
							Else
								Return "4_I"
			...
			Else
				...
				If $pixel[0] > $BRX Then ; slice 2 external
					If ($pixel[0] - $BRX) / ($Right - $BRX) + ($pixel[1] - $RightY) / ($BRY - $RightY) <= 1 Then
						Return "2_E"
					Else
						Return "0_2"
					EndIf
				Else
					If ($BRX - $pixel[0]) / ($BRX - $BottomX) + ($pixel[1] - $RightY) / ($BRY - $RightY) <= 1 Then
						Return "2_I"
					Else
						If ($pixel[0] - $BottomX) / ($BRX - $BottomX) + Abs($BRY - $pixel[1]) / ($BRY - $RightY) <= 1 Then
							If $pixel[1] <= $BRY Then
								Return "1_I"
							Else
								Return "1_E"
